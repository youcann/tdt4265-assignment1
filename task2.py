...
def train(
    ...
    for epoch in range(num_epochs):
        for step in range(num_batches_per_epoch):
            ...
            model.backward(X_batch,model.forward(pre_process_images(X_batch)),Y_batch)
            print(model.grad.shape)
            model.w=model.w-learning_rate*(1/batch_size)*np.sum(model.grad)
